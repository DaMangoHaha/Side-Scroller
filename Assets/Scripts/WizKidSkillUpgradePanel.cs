...
/// <summary>
/// Manages the Wiz Kid Skill Upgrade UI panel.
...
/// </summary>
public class WizKidSkillUpgradePanel : MonoBehaviour
{
    [Header("References")]
    public WizKidSkill wizKidSkill; // assign the player's WizKidSkill component
    ...
    {
        "Tier 1: Sprouting Sorcery Cooldown -3 seconds",
        "Tier 2: Energy per tick +10%, but chance to lose 15% of his max energy",
        "Tier 3: New effects: Grant 50 Coins or 5s Invulnerability"
    };
    ...
    public void OpenPanel()
    {
        ...
        int currentTier = 0;
        if (wizKidSkill != null)
            currentTier = wizKidSkill.GetUpgradeTier();

        panelRoot = new GameObject("WizKidUpgradePanel");
        ...
        titleTMP.text = "Sprouting Sorcery Upgrades";
        titleTMP.fontSize = 36;
        titleTMP.alignment = TextAlignmentOptions.Center;
        titleTMP.color = new Color(0.6f, 0.2f, 1f); // purple-ish for Wiz Kid
    ...
    {
        if (wizKidSkill == null) return;

        int currentTier = wizKidSkill.GetUpgradeTier();
        ...
        {
            Debug.Log("Not enough coins for Wiz Kid Skill Upgrade!");
            ShowNotEnoughCoinsMessage();
            return;
        }

        // Apply upgrade
        wizKidSkill.SetUpgradeTier(tierNumber);

        Debug.Log("Wiz Kid Skill upgraded to Tier " + tierNumber + "!");
    ...
    private Canvas GetOrCreatePopupCanvas()
    {
        ...
        GameObject existing = GameObject.Find("WizKidUpgradeCanvas");
        if (existing != null)
        {
        ...
        GameObject canvasGO = new GameObject("WizKidUpgradeCanvas");
