...
/// <summary>
/// Manages the Ninja Skill Upgrade UI panel.
...
/// </summary>
public class NinjaSkillUpgradePanel : MonoBehaviour
{
    [Header("References")]
    public NinjaSkill_ElectricBolt ninjaSkill; // assign the player's NinjaSkill_ElectricBolt component
    ...
    {
        "Tier 1: Destroying an object with Electric Bolt shortens its cooldown by 3s",
        "Tier 2: Hold activate for 2s to fire a large, slow bolt that pierces obstacles",
        "Tier 3: Using Electric Bolt grants Ninja +10% of her total energy"
    };
    ...
    public void OpenPanel()
    {
        ...
        titleTMP.text = "Electric Bolt Upgrades";
    ...
    {
        if (ninjaSkill == null) return;

        int currentTier = ninjaSkill.GetUpgradeTier();
        ...
        {
            Debug.Log("Not enough coins for Ninja Skill Upgrade!");
            ShowNotEnoughCoinsMessage();
            return;
        }

        // Apply upgrade
        ninjaSkill.SetUpgradeTier(tierNumber);

        Debug.Log("Ninja Skill upgraded to Tier " + tierNumber + "!");
    ...
    private Canvas GetOrCreatePopupCanvas()
    {
        ...
        GameObject existing = GameObject.Find("NinjaUpgradeCanvas");
        if (existing != null)
        {
        ...
        GameObject canvasGO = new GameObject("NinjaUpgradeCanvas");
