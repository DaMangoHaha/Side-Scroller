...
/// <summary>
/// Manages the Selene Skill Upgrade UI panel.
...
/// </summary>
public class SeleneSkillUpgradePanel : MonoBehaviour
{
    [Header("References")]
    public SeleneSkill seleneSkill; // assign the player's SeleneSkill component
    ...
    public void OpenPanel()
    {
        ...
        int currentTier = seleneSkill != null ? seleneSkill.GetUpgradeTier() : 0;

        panelRoot = new GameObject("SeleneUpgradePanel");
        ...
        boxBG.color = new Color(0.12f, 0.05f, 0.15f, 0.95f);
        ...
        titleTMP.text = "Spiritually Charming Upgrades";
        titleTMP.fontSize = 34;
        titleTMP.alignment = TextAlignmentOptions.Center;
        titleTMP.color = new Color(1f, 0.5f, 0.95f); // pink
        ...
        for (int i = 0; i < 3; i++)
        {
            ...
            btnRT.anchoredPosition = new Vector2(0f, -90f - (i * 80f));
            ...
            {
                labelTMP.text = tierDescriptions[i] + "  [OWNED]";
                btnBG.color = new Color(0.15f, 0.4f, 0.15f, 0.9f);
                btn.interactable = false;
            }
            else if (currentTier == tierNumber - 1)
            {
                labelTMP.text = tierDescriptions[i] + "  [" + upgradeCost + " Coins]";
                btnBG.color = new Color(0.3f, 0.1f, 0.35f, 0.9f);
                btn.interactable = true;
                int capturedTier = tierNumber;
                btn.onClick.AddListener(() => PurchaseTier(capturedTier));
                if (firstInteractable == null) firstInteractable = btn;
            }
            else
            {
                labelTMP.text = tierDescriptions[i] + "  [LOCKED]";
                btnBG.color = new Color(0.25f, 0.25f, 0.25f, 0.6f);
                btn.interactable = false;
            }

            ColorBlock colors = btn.colors;
            colors.highlightedColor = new Color(0.5f, 0.2f, 0.6f, 1f);
            colors.pressedColor = new Color(0.25f, 0.1f, 0.3f, 1f);
            colors.selectedColor = new Color(0.5f, 0.2f, 0.6f, 1f);
    ...
    {
        if (seleneSkill == null) return;

        int currentTier = seleneSkill.GetUpgradeTier();
        ...
        seleneSkill.SetUpgradeTier(tierNumber);
        Debug.Log("Selene Skill upgraded to Tier " + tierNumber + "!");
    ...
    private Canvas GetOrCreatePopupCanvas()
    {
        ...
        GameObject existing = GameObject.Find("SeleneUpgradeCanvas");
        if (existing != null)
        {
        ...
        GameObject canvasGO = new GameObject("SeleneUpgradeCanvas");
